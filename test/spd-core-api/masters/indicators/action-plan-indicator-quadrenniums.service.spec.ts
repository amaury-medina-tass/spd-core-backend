...
import { ActionPlanIndicatorQuadrenniumsService } from '../../../../apps/spd-core-api/src/masters/indicators/services/action-plan/action-plan-indicator-quadrenniums.service';
...
describe('ActionPlanIndicatorQuadrenniumsService', () => {
    let service: ActionPlanIndicatorQuadrenniumsService;
    ...
    beforeEach(() => {
        mockQb = createMockQueryBuilder([mockQuad], 1);
        mockQuadRepo = {
        ...
        service = new ActionPlanIndicatorQuadrenniumsService(mockQuadRepo, mockIndicatorRepo, mockAuditLog);
    });
    ...
    describe('findAllPaginated', () => {
        ...
        it('should handle dotted sortBy', async () => {
            await service.findAllPaginated('ind-1', 1, 10, undefined, 'indicator.name', 'ASC');

            expect(mockQb.orderBy).toHaveBeenCalledWith('indicator.name', 'ASC');
        });
        ...
        it('should calculate pagination meta', async () => {
            mockQb.getManyAndCount.mockResolvedValue([[{ id: '1' }], 30]);
