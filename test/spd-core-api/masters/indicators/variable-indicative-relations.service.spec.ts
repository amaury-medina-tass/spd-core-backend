...
import { VariableIndicativeRelationsService } from '../../../../apps/spd-core-api/src/masters/indicators/services/indicative-plan/variable-indicative-relations.service';
...
describe('VariableIndicativeRelationsService', () => {
    let service: VariableIndicativeRelationsService;
    ...
    beforeEach(() => {
        ...
        service = new VariableIndicativeRelationsService(
            mockRelationRepo,
            mockIndicatorRepo,
            mockVariableRepo,
            mockFormulaRepo,
    ...
    describe('associate', () => {
        it('should associate a variable with an indicator', async () => {
        ...
        it('should recalculate formulas after association', async () => {
            mockFormulaRepo.find.mockResolvedValue([{ id: 'f-1', indicativeIndicatorId: 'ind-1' }]);

            await service.associate('ind-1', 'var-1');

            expect(mockFormulaRepo.find).toHaveBeenCalledWith({ where: { indicativeIndicatorId: 'ind-1' } });
    ...
    describe('findPaginated', () => {
        ...
        it('should return available variables', async () => {
            mockRelationRepo.find.mockResolvedValue([{ variableId: 'var-1' }]);
            const mockQb = createMockQueryBuilder([{ id: 'var-2', code: 'V002', name: 'Var 2' }], 1);
            mockVariableRepo.createQueryBuilder.mockReturnValue(mockQb);
