...
import { IndicativeIndicatorUsersService } from '../../../../apps/spd-core-api/src/masters/indicators/services/indicative-plan/indicative-indicator-users.service';

describe('IndicativeIndicatorUsersService', () => {
    let service: IndicativeIndicatorUsersService;
    let mockRepo: any;
    let mockIndicatorRepo: any;
    let mockAuditLog: any;

    const mockIndicator = { id: 'ind-1', code: 'II001', name: 'Indicator 1' };
    ...
    beforeEach(() => {
        mockRepo = {
            ...
            save: jest.fn().mockImplementation((entity: any) => Promise.resolve({ ...entity, id: 'iiu-1' })),
        ...
        service = new IndicativeIndicatorUsersService(mockRepo, mockIndicatorRepo, mockAuditLog);
    });
    ...
    describe('findByIndicatorId', () => {
        it('should return users for an indicator', async () => {
            mockRepo.find.mockResolvedValue([{ id: 'iiu-1', indicatorId: 'ind-1', userId: 'user-1' }]);
    ...
    describe('assign', () => {
        it('should assign a user to an indicator', async () => {
            ...
            const result = await service.assign('ind-1', 'user-1', 'Jane');
        ...
        it('should throw ConflictException if already assigned', async () => {
            mockRepo.findOne.mockResolvedValue({ id: 'iiu-1' });
    ...
    describe('unassign', () => {
        it('should unassign a user from an indicator', async () => {
            mockRepo.findOne.mockResolvedValue({ id: 'iiu-1', indicatorId: 'ind-1', userId: 'user-1' });

            await service.unassign('ind-1', 'user-1', 'Jane');
