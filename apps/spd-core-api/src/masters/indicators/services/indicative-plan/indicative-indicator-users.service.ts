...
@Injectable()
export class IndicativeIndicatorUsersService {
    private readonly logger = new Logger(IndicativeIndicatorUsersService.name);

    constructor(
        @InjectRepository(IndicativeIndicatorUser)
        private readonly repo: Repository<IndicativeIndicatorUser>,
        @InjectRepository(IndicativePlanIndicator)
        private readonly indicatorRepo: Repository<IndicativePlanIndicator>,
    ...
    async findByIndicatorId(indicatorId: string) {
        ...
        if (!indicator) {
            throw new NotFoundException({ message: "Indicador no encontrado", code: ErrorCodes.INDICATIVE_INDICATOR_NOT_FOUND });
        }
    ...
    async assign(indicatorId: string, userId: string, userName?: string) {
        ...
        await this.auditLog.logSuccess(AuditAction.INDICATIVE_INDICATOR_USER_ASSIGNED, AuditEntityType.INDICATIVE_INDICATOR, indicatorId, {
    ...
    async unassign(indicatorId: string, userId: string, userName?: string) {
        ...
        const userDisplayName = userName ?? `Usuario ${userId.substring(0, 8)}`;
        await this.auditLog.logSuccess(AuditAction.INDICATIVE_INDICATOR_USER_UNASSIGNED, AuditEntityType.INDICATIVE_INDICATOR, indicatorId, {
